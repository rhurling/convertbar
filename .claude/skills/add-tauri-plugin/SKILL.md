...
description: Use when adding a Tauri plugin to ConvertBar (autostart, updater, notification, opener, etc.) — runs the official add command and wires up the matching per-call ACL permission so the plugin's frontend API actually works at runtime.
...
3. **Backend-only APIs need no permission** — anything called only from Rust (tray, window management, notifications sent from Rust, opener from Rust) does not require an ACL entry.

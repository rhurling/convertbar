...
Run: `cargo build --workspace --all-targets 2>&1 | grep -E "^error|-->" | head -40`
Expected: FAIL — one `this function takes 4 arguments but 3 arguments were supplied` per site. There should be **14**. If the count differs, stop and reconcile against the spec before continuing.
...
Change it to take the `Ctx` (so it can reach the locator) and update its call sites accordingly:
...
Expected: FAIL. Four tests reach `PanickingLocator`:

| Test | File |
|---|---|
| `purge_bad_sources_destroys_through_the_ctx_disposer` | `crates/convertbar-core/src/queue_ops.rs` |
| `purge_bad_sources_with_no_ids_returns_an_empty_array` | `crates/convertbar-server/src/routes/mod.rs` |
| `detect_handbrake_smoke_returns_200_with_valid_json` | `crates/convertbar-server/src/routes/mod.rs` |
| `add_files_inner_skips_at_target_source_end_to_end` | `crates/convertbar-core/src/queue_ops.rs` (`#[ignore]`d — see Step 7) |

If a *fifth* test fails, that is a genuine discovery, not a mistake: it was machine-coupled too.
Give it `AbsentLocator` and note it in the commit message.
...
```rust
    ...
    async fn detect_handbrake_reports_absent_when_handbrake_is_not_installed() {
        // Pins the CI world: the route must answer 200 with a null path, not 500. Previously
        // this test accepted either answer because it read whatever the host had installed.
        let (state, _tx) =
            test_state_with_locator(Arc::new(convertbar_core::handbrake::AbsentLocator));
        let app = router(state);
        let (status, body) = request_json(app, "GET", "/api/handbrake/detect", None).await;
        assert_eq!(status, 200);
        assert!(body.get("path").map(|p| p.is_null()).unwrap_or(true),
            "absent HandBrake must report a null path, got {body:?}");
    }
    ...
    async fn detect_handbrake_reports_the_located_path_when_handbrake_is_installed() {
        ...
        let (status, body) = request_json(app, "GET", "/api/handbrake/detect", None).await;
        assert_eq!(status, 200);
        assert!(format!("{body}").contains("/opt/fake/HandBrakeCLI"),
            "installed HandBrake must report the located path, got {body:?}");
    }
```

Adjust the two assertions to the route's actual JSON shape — read
`crates/convertbar-server/src/routes/handbrake.rs` for the response type before writing them.

- [ ] **Step 7: Give the `#[ignore]`d e2e test the real world**

`add_files_inner_skips_at_target_source_end_to_end` wants a genuine HandBrakeCLI — it is
`#[ignore]`d and local-only. Build its `Ctx` with `Arc::new(crate::handbrake::PathLocator)` so it
keeps reading the real machine deliberately, which is the one place that is correct.
...
workaround and its comment, and give that test's `Ctx` an explicit `AbsentLocator` or `StubLocator`
as its assertion requires.
...
fn a_queued_job_fails_as_environment_when_handbrake_is_missing() {
    // process_queue resolves HandBrake per job. With it absent the job must be recorded as an
    // Environment failure and the queue must move on — not hang, and not be retried forever.
    let conn = test_conn();
    insert_queued(&conn, "j1", "/tmp/a.mkv", "queued", 0);
    let (ctx, sink, _d) = test_ctx_with_locator(conn, Arc::new(crate::handbrake::AbsentLocator));

    process_queue(&ctx);

    let (status, class, error): (String, Option<String>, Option<String>) = ctx
        .db
        .lock()
        .unwrap()
        .query_row(
            "SELECT status, failure_class, error FROM jobs WHERE id = 'j1'",
            [],
    ...
    assert_eq!(class.as_deref(), Some("environment"));
    assert!(
        error.unwrap_or_default().contains("HandBrakeCLI not found"),
        "the failure reason must name the missing binary so the user can act on it"
    );
    let _ = sink;
}
```
...
Expected: PASS. Reconcile the exact `failure_class` string and column names against
`crates/convertbar-core/src/failure_class.rs` and the `jobs` schema in `db.rs` — adjust the
assertion to the real values rather than changing production code to match the test.
...
A guard that cannot fail is not a guard. Temporarily change the `test_ctx` default in
`crates/convertbar-core/src/queue_ops.rs` from `PanickingLocator` to `PathLocator`, then:

Run: `cargo test -p convertbar-core --lib add_files_inner_reports_handbrake_missing`
Expected on a machine **with** HandBrake: the absent-world test FAILS. That is the proof the strict
default is load-bearing.

Revert the mutation: `git checkout crates/convertbar-core/src/queue_ops.rs`

**Commit any real work before this step** — the `git checkout` discards uncommitted changes in that
file.
...
- Task 3 Step 6 and Task 4 Step 6 cannot list every assertion in advance, because they depend on the
  server route's JSON shape and on which route tests reach the guard once the fixture pin is gone.
  Both steps say to read the actual response type first and adapt. This is deliberate — inventing
  exact assertions for code I have not read would be a worse failure than naming the uncertainty.
- Task 5's `failure_class` string (`"environment"`) is my reading of the `Environment` variant's
  serialization; Step 3 instructs verifying it against `failure_class.rs` rather than trusting it.

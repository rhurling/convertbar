...
- **Confirmed** — [Medium] useQueue multi-event refresh race (useQueue.ts:27-38): converter.rs emits job-completed + job-status-changed on completion (converter.rs:617-633) and the next job's start adds job-status-changed(encoding) (converter.rs:322-328), so 3 listened events can fire near-simultaneously, each triggering an unguarded independent `getQueue()` (useQueue.ts:10-17) with no request ordering — a stale snapshot can win.
- **Confirmed** — [High] non-optimistic `updateSetting` (useSettings.ts:66-89): verified it awaits `update_setting` then a full `get_settings` before `setSettings` (useSettings.ts:68-70); SettingsPage binds `value={settings.handbrake_path}` / `value={settings.watch_skip_marker}` directly to hook state with no local draft (SettingsPage.tsx:311, 232), so the controlled value cannot advance until the 2-IPC round-trip resolves, and interleaved keystrokes' out-of-order `get_settings` responses can revert the field.
- **Confirmed** — [Medium] `refresh()` write side effect (useSettings.ts:42-52): on suffix miss it calls the `set_preset_suffix` write command (useSettings.ts:47); the backend's `get_preset_suffix` returns `Ok(None)` when unset rather than a default (settings.rs:149-152), so the default truly lives in the frontend read path, which runs twice under StrictMode (useSettings.ts:62-64); the same fallback block is duplicated at useSettings.ts:72-83.
- **Confirmed** — [Medium] DropZone folder-confirm "Add" un-caught (DropZone.tsx:82-93): the async onClick has no try/catch; `setPendingFolders(remaining)` sits after the awaited `confirmFolderAdd`, so on rejection the row stays, no status is set, and the rejection is unhandled — the click appears dead, exactly as described.
- **Confirmed** — [Medium] `pauseAfter` desync (ActiveJob.tsx:12): backend flag is `ConverterState.pause_after_current` (converter.rs:103) with only setter/clearer commands registered (lib.rs:71-72) — no getter, `queue-updated` payload is `()` (watcher.rs:327), and `PlatformCapabilities` exposes only `can_pause_process` (commands/converter.rs:330-338). Both arm paths verified: SettingsPage updater flow calls `pauseAfterCurrent()` (SettingsPage.tsx:352), and tab-switching unmounts ActiveJob (App.tsx:39-42) resetting `useState(false)` while the flag stays armed. Bonus third path: on non-macOS, `pause_conversion` itself sets the flag (commands/converter.rs:36-40); the converter also clears it unilaterally on consume (converter.rs:681-682), so desync runs both directions.
- **Confirmed** — [Medium] `fileName()` splits on "/" only (format.ts:22-24): backend job payloads carry OS-native path strings — `source_path` comes straight from the DB (queue.rs:59), populated via `to_string_lossy()`/`Path::join` (e.g. watch.rs:174, queue.rs:156-176) with no separator normalization anywhere in src-tauri, so Windows paths arrive with backslashes and `split("/")` returns the full path. WatchedFoldersPage.tsx:5-8 indeed has the separator-agnostic `split(/[/\\]/)` version, confirming the duplication.
- **Confirmed** — [High] per-keystroke IPC on three SettingsPage text inputs (SettingsPage.tsx:308-317, 228-234, 128-135): all three are controlled purely by hook state (no local echo state exists in SettingsPage — its only useState is updateStatus/appVersion), with onChange → `updateSetting` (2 awaited IPCs before setState) or `updatePresetSuffix` (1 awaited IPC before setState, useSettings.ts:91-98). A backend write fires per keystroke and the rendered value lags the round-trip; the WatchRow draft/commit pattern the report recommends genuinely exists (WatchedFoldersPage.tsx WatchRow).
- **Confirmed** — [Medium] `onHbPathChanged` races the settings write (SettingsPage.tsx:312-315): `updateSetting` is fired un-awaited and `onHbPathChanged()` is called synchronously after; it invokes `validate_handbrake`, which reads `handbrake_path` from the DB (commands/handbrake.rs:88-96), so if the read is processed before the in-flight `update_setting` write commits, the banner reflects the old path.
- **Confirmed** — [Medium] `resolveTemplate` diverges from Rust (SettingsPage.tsx:20-41 vs handbrake.rs:212-264): algorithms differ as claimed (JS: global replace then regex cleanup, with `[-_]\.` / `\.[-_]` lacking `/g`; Rust: per-empty-var removal of one adjacent `-`/`_`/`.` separator via `replacen`, no `[-_]{2,}` collapse). Concrete divergent input: template `.{resolution}.{codec}` with resolution="" and codec="h265" → JS yields `..h265` (none of its cleanup regexes match consecutive dots) while Rust yields `.h265` (`{resolution}.` removed as var+sep) — preview shows `vacation..h265.mp4`, actual file is `vacation.h265.mp4`. Second example: `{quality}-{device}-{codec}` with quality/device empty → JS `-h265` (leading dash kept after `[-_]{2,}` collapse), Rust `h265`.

**Tally:** 9 confirmed, 0 partial, 0 refuted (of 9).

...
- **Confirmed** — [Medium] watched path stored verbatim, UNIQUE only byte-identical (watch.rs:45-60): verified — `path` is inserted as received (watch.rs:60) against `path TEXT NOT NULL UNIQUE` (db.rs:53) with no canonicalization anywhere, and `reconcile` arms one watcher per row (watcher.rs:463-479). Mitigating note: the sole UI entry point is the native picker (`useWatchedDirectories.ts:37` -> `pick_folder`), which never emits trailing slashes, so the realistic alias vector is symlinked paths (e.g. `/tmp` vs `/private/tmp`), not user-typed variants.
- **Confirmed** — [Medium] `reconcile` never purges `pending` under removed/disabled roots (watcher.rs:440-483): verified end-to-end — `reconcile` touches only configs/skip_marker/watched/watcher, never `pending`; the reaper enqueues any settled entry with no config re-check (watcher.rs:263-278); `enqueue_and_start` (308-328) applies only `filter_marked`, and `has_active_marker` returns `false` for paths outside every watched root (watcher.rs:122-128), so a file under a removed root passes straight into `add_files_inner`, whose skip rules test duplicates/history, not watch membership.
- **Partial** — [High] `preset_cache` mutex held across `get_preset_metadata` shell-out, contended by sync `generate_preset_suffix` (queue.rs:335-344, 363-372): lock-convoy mechanism confirmed — the guard is held across `Command::output()` (handbrake.rs:65-71) at both sites; `generate_preset_suffix` is a sync command (commands/handbrake.rs:42) locking the same mutex at :48, is really invoked by the frontend (src/lib/tauri.ts:143), and watcher adds run `add_files_inner` on background threads (watcher.rs:426, 507). Correction: "seconds" is overstated — `HandBrakeCLI --preset-export` measured ~0.19s (no file scan involved), so the stall is a brief hitch, arguing Medium rather than High. Also note `generate_preset_suffix` itself shells out on the main thread on a cache miss (handbrake.rs:76) — a more direct instance of the same class, unflagged.
- **Refuted** — [Medium] hardcoded `.mp4` yields Matroska data in `.mp4`-named files on Linux (queue.rs:163, 175): the premises hold (`.mp4` hardcoded, Linux default "H.265 MKV 1080p" at db.rs:17, converter passes no `--format` at converter.rs:371-378), but the mechanism does not — HandBrakeCLI auto-detects the container from the destination extension when `--format` is absent (`--help`: "default: auto-detected from destination file name"), verified empirically with HandBrake 1.11.2: `-Z "H.265 MKV 1080p30" -o out.mp4` produced an MP4 container (`+ container: MPEG-4 (libavformat)`, ffprobe `format_name=mov,mp4`). The real Linux-default bug found while testing is different: "H.265 MKV 1080p" is not a valid preset name in current HandBrake ("Invalid preset" — the real name is "H.265 MKV 1080p30"), so every conversion with the Linux default errors out entirely.
- **Confirmed** — [Medium] `scan_folder`/`classify_paths` run unbounded recursive walks as sync commands (queue.rs:560, 810): verified — both are non-async `#[tauri::command]` fns (main thread per Tauri's sync-command dispatch, as this codebase's own comments at watch.rs:163-166 and queue.rs:548-550 rely on) calling `scan_video_files` (queue.rs:32-45), a recursive walk with per-entry `is_dir` stats and no depth bound; siblings `add_files`/`confirm_folder_add` (queue.rs:547, 581) use `spawn_blocking`, confirming the intended pattern these two miss.

**Tally:** 3 confirmed, 1 partial, 1 refuted (of 5).
